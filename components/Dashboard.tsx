...
const Dashboard: React.FC<DashboardProps> = ({ user, onViewCourse, onLaunchTribe, onViewHRReport }) => {
  ...
  const scores = user.pillarScores || {
    engine: 50, fuel: 20, voice: 60, brain: 80, pulse: 40, shield: 30, tribe: 0
  };
  ...
  const weightedScore = (
     (scores.fuel * 1.5) + (scores.engine * 1.2) + (scores.voice * 1.2) + 
     scores.brain + scores.pulse + scores.shield + scores.tribe
  ) / 7.9; // Sum of weights
  ...
  const LEVER_MAPPING: Record<string, string[]> = {
     fuel: ['costControl', 'price', 'retention'],
     engine: ['efficiency', 'teamSpeed', 'costControl'],
     voice: ['conversion', 'retention', 'price'],
     pulse: ['quality', 'conversion', 'retention'],
     shield: ['riskReduction', 'costControl', 'teamSpeed'],
     brain: ['teamSpeed', 'efficiency', 'conversion'],
     tribe: ['teamSpeed', 'efficiency', 'retention']
  };
