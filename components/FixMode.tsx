...
const FixMode: React.FC<FixModeProps> = ({ user, onUpdateUser, onBack }) => {
  ...
  const handleCompleteTask = async () => {
    ...
    setTimeout(() => {
      ...
      const currentScores = user.pillarScores || { engine: 0, fuel: 0, voice: 0, brain: 0, pulse: 0, shield: 0, tribe: 0 };
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${
          task.pillar === 'Shield' ? 'bg-red-50 text-red-700 border-red-100' : 
          task.pillar === 'Fuel' ? 'bg-green-50 text-green-700 border-green-100' :
